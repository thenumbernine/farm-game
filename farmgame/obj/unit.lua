...
function Unit:init(args, ...)
	...
	self.items = table{
		require 'farmgame.item.sword',
		require 'farmgame.item.pickaxe',
		require 'farmgame.item.shovel',
		require 'farmgame.item.axe',
		require 'farmgame.item.hoe',
		require 'farmgame.item.wateringcan',
		require 'farmgame.item.fishingpole',
	}:mapi(function(cl)
		return {
			class = cl,
			count = 1,
...
print'YOU CAUGHT A FISH'
			...
			self.map:newObj{
				class = require 'farmgame.obj.item',
				itemClass = require 'farmgame.obj.fish',
				pos = self.pos:clone(),
			}
		end
	end
